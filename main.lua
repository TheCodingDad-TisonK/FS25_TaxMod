...
local function applyDailyTax()
    ...
    if settings.showNotification then
        g_currentMission:addIngameNotification({1.0, 0.0, 0.0, 1.0},
            string.format("Daily tax deducted: -%s", formatMoney(taxAmount)))
    end
end
...
local function applyMonthlyReturn()
    ...
    local farmId = g_currentMission:getFarmId()
    if not farmId then return end
    local returnAmount = math.floor(stats.taxesThisMonth * (math.min(math.max(settings.returnPercentage,0),100)/100))
    if returnAmount > 0 then
        ...
        if taxHUD then
            local env = g_currentMission.environment
            taxHUD:recordTax(returnAmount, env and env.currentDay or 0, env and env.currentMonth or 0, true)
        end
        if settings.showNotification then
            g_currentMission:addIngameNotification({0.0, 1.0, 0.0, 1.0},
                string.format("Monthly tax return: +%s", formatMoney(returnAmount)))
        end
    end
...
function FS25TaxMod:consoleTaxStatistics()
    ...
    print("This month:      " .. formatMoney(stats.taxesThisMonth))
...
function taxReturn(p)    FS25TaxMod:consoleTaxReturn(p)    end
